// 2D Donut Chart (Mini Chart) for rotation control
export class Donut2D {
    ...
    handleDrag(event) {
        ...
        this.rotation += delta;
        this.group.attr('transform', `translate(${center.x},${center.y}) rotate(${this.rotation * 180 / Math.PI})`);
    ...
    update(data) {
        ...
        const enterPaths = paths.enter()
            .append('path')
            .attr('fill', (d, i) => this.options.colorScale(i))
            .attr('opacity', 0);

        this.svg.on('mouseover', () => {
            const tooltipContent = pieData
                .map(d => `${d.data.name}: ${(((d.endAngle - d.startAngle) / (2 * Math.PI)) * 100).toFixed(1)}%`)
                .join('<br>');
            const containerRect = this.container.node().getBoundingClientRect();
            this.tooltip
                .style('opacity', 1)
                .style('left', `${containerRect.left + this.options.width}px`)
