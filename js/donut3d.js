// 3D Donut Chart for Steam Games Data Visualization
...
export class Donut3D {
    constructor(selector, options = {}) {
        ...
        // Steam color palette
        const steamColors = [
            '#66c0f4', '#1b2838', '#2a475e', '#c7d5e0', '#a4c7e7',
            '#1e3a52', '#8bb8d8', '#4e79a7', '#6fa8d0', '#5c8bb5'
        ];
    ...
    setupVisualization(selector) {
        ...
        this.gameDetails.append('h2').text('Selected Game Information');
        this.gameDetailsContent = this.gameDetails.append('div').attr('class', 'vis1-game-details-content');
    ...
    createDonutShapes(data, innerR, outerR, heightScale = 1, segmentsCount = 1) {
        ...
        data.forEach((item, index) => {
            ...
            for (let i = 0; i < segmentsCount; i++) {
                ...
                const points = this.createSegmentPoints(startAngle, endAngle, innerR, outerR, currentHeight);
                this.addShapesToLists(points, index, i, trianglesList, planesList, this.options.colors(index));
            }
            angle += segmentAngle;
        });
    ...
    addShapesToLists(points, segmentIndex, subIndex, triangles, planes, color) {
        const { outerTop1, outerTop2, innerTop1, innerTop2, outerBottom1, outerBottom2, innerBottom1, innerBottom2 } = points;

        // Planes
        this.addPlane(planes, [outerTop1, outerTop2, outerBottom2, outerBottom1], `seg-${segmentIndex}-outer-${subIndex}`, color);
        this.addPlane(planes, [innerTop2, innerTop1, innerBottom1, innerBottom2], `seg-${segmentIndex}-inner-${subIndex}`, color);
        this.addPlane(planes, [outerTop1, innerTop1, innerTop2, outerTop2], `seg-${segmentIndex}-top-${subIndex}`, color);
        this.addPlane(planes, [outerBottom2, innerBottom2, innerBottom1, outerBottom1], `seg-${segmentIndex}-bottom-${subIndex}`, color);

        // Triangles (caps)
        this.addTriangle(triangles, [outerTop1, innerTop1, innerTop2], `seg-${segmentIndex}-toptri-${subIndex}-a`, color);
        this.addTriangle(triangles, [outerTop1, innerTop2, outerTop2], `seg-${segmentIndex}-toptri-${subIndex}-b`, color);
    }
    ...
    setRotation(angleRad) {
        ...
        this.animationFrame = requestAnimationFrame(() => {
            const rotated = {
                triangles: this.triangles3d.rotateX(this.rotationX + this.options.startRotationX).rotateY(this.rotationY + this.options.startRotationY)(this.triangles),
                planes: this.planes3d.rotateX(this.rotationX + this.options.startRotationX).rotateY(this.rotationY + this.options.startRotationY)(this.planes)
            };
    ...
    update(data, options = {}) {
        ...
        const shapes = this.createDonutShapes(data, config.innerRadius, config.outerRadius, config.heightScale, config.segments);
    ...
    updateShapes(shapes, transitionDuration) {
        this.updatePathElements(shapes.triangles, 'caps', this.triangles3d.draw, transitionDuration);
        this.updatePathElements(shapes.planes, 'faces', this.planes3d.draw, transitionDuration);
        this.group.selectAll('.d3-3d').sort(this.triangles3d.sort);
        if (this.selectedSegmentId !== null) this.highlightSegment(this.selectedSegmentId);
    }

    updateShapesOptimized(shapes) {
        if (this.isRotating) {
            if (this.hoveredSegment) this.hoveredSegment = null;
    ...
    updatePathElementsOptimized(data, className, drawFunction) {
        this.group.selectAll(`path.${className}`).data(data, d => d.id).attr('d', drawFunction);
    }
    ...
    handleSegmentHover(d, isHover) {
        ...
        if (isHover) {
            if (segmentId !== this.hoveredSegment) this.setHoverState(segmentId);
        } else {
            if (segmentId === this.hoveredSegment) this.setHoverState(null);
        }
    }

    setHoverState(segmentId) {
        if (this.hoveredSegment === segmentId) return;
        if (this.hoveredSegment !== null) this.applySegmentStyle(this.hoveredSegment, 'normal');
    ...
    applySegmentStyle(segmentId, state) {
        const paths = this.group.selectAll('path').filter(pd => pd.id.split('-')[1] === segmentId);
        switch (state) {
            case 'hover':
                if (segmentId !== this.selectedSegmentId) {
                    paths.style('filter', 'brightness(1.25)').style('opacity', 0.9);
                }
                break;
            case 'selected':
    ...
    updateGameDetails(data) {
        ...
        if (data) {
            const gameInfo = this.gameDetailsContent.append('div').attr('class', 'vis1-game-info');
            gameInfo.append('h3').text(data.name);
            const details = [
            ...
            details.forEach(detail => {
                ...
                if (detail.desc) {
                    p.append('span').attr('class', 'vis1-detail-desc').text(' ' + detail.desc);
                }
            });
        } else {
            this.gameDetailsContent.append('p').attr('class', 'vis1-placeholder').text('Click on a game segment to view details');
        }
    }
    ...
    destroy() {
        ...
        if (this.svg) this.svg.on('.drag', null);
        this.isRotating = false;
        this.triangles = [];
        this.planes = [];
        this.currentData = null;
